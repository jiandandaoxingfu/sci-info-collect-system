class Spider {
	init(title_arr, author_arr, threads, sid, tabid) {
	...
	get_cite_data(id) {
		...
		return axios.get(cite_url).then( res => {
			...
			if( records ) {
					...
					this.cite_num_arr[id] = [records.length, 0];
			} else {
				this.search_states[id][1] = -1;
				this.search_states[id][4] = 1;
			}
	...
	get_detail_data(id) {
		...
		return axios.get(detail_url).then( res => {
			...
			console.log(id + 1 + ' : ' + '以及完成详情页处理' + new Date().getMinutes() + ':' + new Date().getSeconds() );
		})
	}
	...
	get_journal_data(id) { 
		...
		return axios.get(journal_info_url).then( res => {
			...
			console.log(id + 1 + ' : ' + '以及完成期刊分区处理' + new Date().getMinutes() + ':' + new Date().getSeconds() );
		})
	}
